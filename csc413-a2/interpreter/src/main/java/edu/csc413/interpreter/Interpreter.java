...
public class Interpreter {
    ...
    private Statement parsePrintStatement(String line) {
        ...
        if (printMatcher.matches()) {
            Expression expressionLine = Expression.create(printMatcher.group(1).strip());
            return new PrintStatement(expressionLine);
    ...
    private Statement parseAssignStatement(String line) {
        ...
        if (assignMatcher.matches()) {
            ...
            Expression expressionLine = Expression.create(assignMatcher.group(2).strip());
            return new AssignStatement(variableName, expressionLine);
    ...
    private Statement parseForStatement(String line, Queue<String> lines) {
        ...
        if (forMatcher.matches()) {
            ...
            Condition condition =  Condition.create(conditionLine);
            List<Statement> loopStatements = parseBlockStatements(lines);
            Statement statement = parseAssignStatement(initializationLine);
            Statement statementUp = parseAssignStatement(updateLine);
            return new ForStatement(loopStatements, condition,statement, statementUp);
